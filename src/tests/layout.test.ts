...
describe("calculateSlots", () => {
  ...
  it("asymmetric margins produce correct slot positions", () => {
    ...
    // top: 5 + 4.25 = 9.25mm, bottom: 5 + 5.5 = 10.5mm, left: 5 + 3.4 = 8.4mm, right: 5 + 3 = 8mm
    expect(slots[0].x).toBe(mmToPx(8.4));
    expect(slots[0].y).toBe(mmToPx(9.25));
    expect(slots[0].width).toBe(postcard.widthPx - mmToPx(8.4) - mmToPx(8));
